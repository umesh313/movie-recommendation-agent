import { useEffect, useState } from "react";
import { Star, TrendingUp } from "lucide-react";
import { motion } from "framer-motion";
import { featuredMovies, getShuffledFeaturedMovies } from "@/data/featuredMovies";
import { FeaturedMovieModal } from "@/components/movies/FeaturedMovieModal";
import { posterUrl, searchMovies } from "@/services/tmdb";
import type { FeaturedMovie as FeaturedMovieType } from "@/data/featuredMovies";

interface MovieWithPoster extends FeaturedMovieType {
  posterUrl?: string | null;
  tmdbId?: number;
}

export function FeaturedMovies() {
  const [movies, setMovies] = useState<MovieWithPoster[]>([]);
  const [selectedMovie, setSelectedMovie] = useState<FeaturedMovieType | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Get shuffled movies on mount
    const shuffled = getShuffledFeaturedMovies(10);
    setMovies(shuffled.map(movie => ({ ...movie })));
    setLoading(false);

    // Fetch posters for each movie
    fetchPosters(shuffled);
  }, []);

  async function fetchPosters(moviesList: FeaturedMovieType[]) {
    // Fetch posters with a small delay between requests to avoid rate limiting
    const updatedMovies: MovieWithPoster[] = moviesList.map(movie => ({ ...movie }));
    
    for (let i = 0; i < moviesList.length; i++) {
      try {
        const results = await searchMovies(moviesList[i].title);
        if (results.length > 0) {
          updatedMovies[i].posterUrl = posterUrl(results[0].poster_path, "w342");
          updatedMovies[i].tmdbId = results[0].id;
        }
      } catch (error) {
        console.error(`Error fetching poster for ${moviesList[i].title}:`, error);
      }
      
      // Add small delay to avoid rate limiting
      if (i < moviesList.length - 1) {
        await new Promise(resolve => setTimeout(resolve, 100));
      }
    }
    
    setMovies(updatedMovies);
  }

  function handleClick(movie: FeaturedMovieType) {
    setSelectedMovie(movie);
    setIsModalOpen(true);
  }

  function handleCloseModal() {
    setIsModalOpen(false);
    setTimeout(() => setSelectedMovie(null), 300);
  }

  if (loading) {
    return (
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 bg-gradient-to-br from-cinema-gold/20 to-cinema-gold/5 rounded-xl">
            <TrendingUp className="h-6 w-6 text-cinema-gold animate-pulse" />
          </div>
          <div>
            <h2 className="text-2xl font-bold">Featured Movies</h2>
            <p className="text-sm text-muted-foreground">
              Loading curated recommendations...
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
          {[...Array(10)].map((_, i) => (
            <div key={i} className="aspect-[2/3] bg-gray-800/50 rounded-xl animate-pulse" />
          ))}
        </div>
      </div>
    );
  }

  return (
    <>
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-6">
          <div className="p-2 bg-gradient-to-br from-cinema-gold/20 to-cinema-gold/5 rounded-xl">
            <TrendingUp className="h-6 w-6 text-cinema-gold" />
          </div>
          <div>
            <h2 className="text-2xl font-bold">Featured Movies</h2>
            <p className="text-sm text-muted-foreground">
              Handpicked highly-rated films (IMDb 7.8-7.9) • Click to explore
            </p>
          </div>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
          {movies.map((movie, index) => (
            <motion.div
              key={movie.rank}
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: index * 0.05 }}
              onClick={() => handleClick(movie)}
              className="group relative glass rounded-xl overflow-hidden border border-white/5 hover:border-cinema-gold/30 transition-all duration-300 hover:shadow-lg hover:shadow-cinema-gold/10 cursor-pointer"
            >
              {/* Poster or Placeholder */}
              <div className="aspect-[2/3] relative overflow-hidden bg-gradient-to-br from-gray-800 to-gray-900">
                {movie.posterUrl ? (
                  <>
                    <img
                      src={movie.posterUrl}
                      alt={movie.title}
                      className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                      loading="lazy"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/30 to-transparent opacity-60 group-hover:opacity-80 transition-opacity" />
                  </>
                ) : (
                  <>
                    <div className="absolute inset-0 flex items-center justify-center">
                      <span className="text-5xl font-bold text-white/5">#{movie.rank}</span>
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent" />
                  </>
                )}
              </div>
              
              {/* Content */}
              <div className="absolute bottom-0 left-0 right-0 p-3">
                <h3 className="font-semibold text-sm leading-tight mb-2 line-clamp-2 drop-shadow-lg">
                  {movie.title}
                </h3>
                
                <div className="flex items-center justify-between text-xs">
                  <span className="text-white/70">{movie.year}</span>
                  <div className="flex items-center gap-1 text-cinema-gold">
                    <Star className="h-3 w-3 fill-current" />
                    <span className="font-medium">{movie.imdbRating}</span>
                  </div>
                </div>
                
                <div className="text-xs text-white/50 mt-1">
                  {movie.votes} votes
                </div>
              </div>

              {/* Rank Badge */}
              <div className="absolute top-2 right-2 px-2 py-1 bg-cinema-gold/20 backdrop-blur-sm rounded-full text-xs text-cinema-gold font-medium">
                Top {movie.rank}
              </div>

              {/* Hover Overlay */}
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <div className="bg-black/60 backdrop-blur-sm px-4 py-2 rounded-full text-sm text-white font-medium">
                  Click to explore
                </div>
              </div>
            </motion.div>
          ))}
        </div>

        <div className="mt-6 text-center">
          <p className="text-sm text-muted-foreground">
            Showing {movies.length} of {featuredMovies.length} featured films • 
            Ask for specific genres or ratings to get personalized recommendations
          </p>
        </div>
      </div>

      {/* Movie Details Modal */}
      {selectedMovie && (
        <FeaturedMovieModal
          movie={selectedMovie}
          isOpen={isModalOpen}
          onClose={handleCloseModal}
        />
      )}
    </>
  );
}